...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun withdraw(index: Int, amount: Long): Long {
        ...
        while (true) {
            ...
            val diff = account.amount - amount
            check(diff >= 0) { "Underflow" }
            val updated = Account(diff)
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             * Basically, implementation of this method must perform the logic of the following code "atomically":
             */
            val (from: AcquiredAccount?, to: AcquiredAccount?) = acquireFromTo()
        ...
        private fun acquireFromTo(): Pair<AcquiredAccount?, AcquiredAccount?> {
            return if (fromIndex < toIndex) {
                ...
                (acquire(toIndex, this) to acquire(fromIndex, this)).swap()
            }

        }

        private fun <A, B> Pair<A, B>.swap(): Pair<B, A> = second to first
    }
}
